...
def evaluate_latency_across_sizes(network_sizes=[2, 4], packets_per_node=3, packet_size=3):
    """
    Simplified evaluation of how latency changes with network size.
    """
    results = {}
...
def evaluate_throughput_vs_injection(network_size=4, injection_rates=[0.05, 0.1, 0.2, 0.3], packet_size=3, simulation_time=500):
    """
    Simplified evaluation of throughput at different injection rates.
    """
    results = {}
    ...
    for rate in injection_rates:
        ...
        # Generate traffic with this injection rate
...
def evaluate_exb_performance(network_size=4, exb_sizes=[1, 5, 10], packet_size=5):
    ...
    for exb_size in exb_sizes:
        ...
        # Set the EXB size for all nodes
        for nodeID, node in noc.nodes.items():
            node.exb = simpy.Store(noc.env, capacity=exb_size)
        
        # Log the structure of exb.put for debugging
        print(f"DEBUG: Type of node.exb.put: {type(noc.nodes[0].exb.put)}")
        print(f"DEBUG: Dir of node.exb.put: {dir(noc.nodes[0].exb.put)}")
        
        # Count usage without using lambda
        exb_put_count = 0
        
        # Define a better monitoring approach
        def create_counter():
            counter = 0
            def count_puts(item):
                nonlocal counter
        ...
        for nodeID, node in noc.nodes.items():
            ...
            # Don't modify the actual put method yet
        
        # Generate traffic that will cause EXB contention
        # Send just a few test packets to debug
        for src in range(2):  # Just use 2 nodes for testing
            for i in range(2):  # Just 2 packets per node
        ...
        try:
            ...
            # Try to run without any modifications as a comparison
...
def evaluate_traffic_patterns(network_size=4, injection_rate=0.1, packet_size=3, simulation_time=300):
    """
    Simplified evaluation of different traffic patterns.
    """
    results = {}
    ...
    for pattern_name, dest_function in traffic_patterns.items():
        ...
        for src in range(total_nodes):
            ...
            while next_time < simulation_time and packet_count < max_packets:
                ...
                if next_time < simulation_time:
                    # Get destination based on traffic pattern
...
if __name__ == "__main__":
    # Run validation tests first to ensure the code is working correctly
    ...
    # 4. Traffic Patterns
    patterns = list(results['traffic'].keys())
    traffic_latencies = [results['traffic'][pattern]['avg_latency'] for pattern in patterns]
    traffic_throughputs = [results['traffic'][pattern]['throughput'] for pattern in patterns]
    
    plt.figure(figsize=(10, 5))
    plt.bar(patterns, traffic_latencies, color='salmon')
    plt.xlabel('Traffic Pattern')
    plt.ylabel('Average Latency (cycles)')
    plt.title('Latency by Traffic Pattern')
    plt.xticks(rotation=45)
    plt.grid(axis='y')
    plt.tight_layout()
    plt.savefig('latency_by_pattern.png', dpi=300)
    plt.show()
